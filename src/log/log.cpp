...
Log::Log()
{
    ...
    snprintf(filePath, sizeof(filePath) - 1, "./.log_file/%d_%02d_%02d_%02d:%02d:%02d.log",
             lt->tm_year + 1900, lt->tm_mon + 1, lt->tm_mday, (lt->tm_hour + 8) % 24, lt->tm_min, lt->tm_sec);        // log file path
    ...
    // 启动日志线程
    _logThread1 = std::thread(&Log::logThreadFunc1, this, 1);   //   wait thread completed
